...
void main()
{
   ...
   // Surface Normal
   vec3 normal = normalize(cross(dFdx(outFragPos), dFdy(outFragPos)));
    ...
    illuminated_color += diffuse_lighting(light_position, outFragPos, normal, 1.0, light_color);
    illuminated_color += specular_lighting(light_position, eye_position, outFragPos, normal, 32.0, 20.0, light_color);
